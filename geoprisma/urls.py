...
urlpatterns = patterns('',
    ...
    url(r'^proxy/(?P<service_slug>[\w-]+)/(?P<resource_slug>[\w-]+)/(?P<layer>[\w-]+)$', views.get_proxy),
    url(r'^proxy/(?P<service_slug>[\w-]+)/(?P<resource_slug>[\w-]+)/(?P<layer>[\w-]+)/(?P<data_id>[\w-]+)$', views.get_proxy),
